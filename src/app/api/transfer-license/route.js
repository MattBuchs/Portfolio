...
import { LicenseTransferEmail } from "@/lib/email-templates";
...
export async function PATCH(request) {
	try {
		...
		// Envoyer un email de notification de transfert
		try {
			await resend.emails.send({
				from: FROM_EMAIL,
				to: getRecipientEmail(matchingLicense.email),
				subject: "GameMaster OS - Transfert de licence effectué",
				react: LicenseTransferEmail({
					customerName: matchingLicense.customerName,
					email: matchingLicense.email,
					licenseKey,
					plan: matchingLicense.plan,
					remainingUsages:
						matchingLicense.maxUsages -
						updatedLicense.remainingUsages,
					machineId,
				}),
			});
		} catch (emailError) {
